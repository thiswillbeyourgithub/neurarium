...
# Connectivity node literals + the pathway quote-source constants the
# PROJECTION_QUOTES map below cites (defined in data_generators.connectivity to
# keep this module smaller; no import cycle, connectivity imports only provenance).
...
# several pathways (e.g. one naming the whole striatal output) is written once as a
# ``_KQ_*`` constant (those reused by CIRCUITS/PROJECTION_GROUPS live in
# data_generators.connectivity and are imported above), not duplicated per entry.
