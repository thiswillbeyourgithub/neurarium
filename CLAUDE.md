...
Two palettes, one token block. `index.html`'s `:root` defines the dark palette; a `:root[data-theme="light"]` block redefines **only tokens**, never a rule, so a component written later is themed on the day it lands as long as it reads them. The one that makes this cheap is **`--ink`**, an RGB triplet rather than a colour: every white-on-dark hairline, hover fill and divider is `rgba(var(--ink), a)`, so flipping the chrome is one token instead of dozens of edits. Alongside it `--page-bg`, `--text-strong`, `--label-ink` / `--label-halo` (the floating region labels sit over the scene, not the panel, so they carry their own ink) and `--mol-filter` (the molecule line art is inverted on dark, left alone on light). The same split covers the **semantic** ink: `--sem-missing` / `--sem-llm` / `--sem-sourced` / `--sem-verified` / `--sem-uncertain` / `--sem-amber` / `--sem-caution` (the graded pills and flag tags), `--tip-bg` / `--tip-shadow` (the tooltip bubble, opaque and a step darker than the panel it covers) and, for a boxed callout, `--addon-ink` (split from `--addon-accent` because the accent paints the callout's own ground, so it cannot also be its text). A `#fff` sitting **on** `var(--accent)` stays literal: it is white in both themes, and `tools/tests/test_theme_tokens.py` fails on any other literal text colour, plus on a token one theme declares and the other does not.
...
**A verdict is about a claim, not a sentence (the citation site).** A quote id is a content hash, so
